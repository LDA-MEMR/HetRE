############################################################################
# Function for MCEM under HET model
############################################################################
...
MCEM.HET <- function(n, q, data, M=1000, tol = 0.0005, theta.old, burnin = 500, alpha.old, mu_f=0, var_f= 1)
{ 
  burnin <- burnin
  Mm <- M-burnin
  k <- 0
  ...
  varu <- c(theta.k[5],theta.k[6],theta.k[7],theta.k[8])
  alphaA <- c(theta.k[9], theta.k[10],theta.k[11],theta.k[12])
  alpha0 <-  theta.k[9]
  alpha <- c(theta.k[10],theta.k[11],theta.k[12])
  Zij <- data$Zij
  Zistar <- data$Zistar
  eij <- data$eij
  Zii <- matrix(data$Zistar,n,q)
  ## Indicator variable to generate GARP and IV parameters
  I <- matrix(NA,q,q)
  for (j in 1:q)
  {
    for (t in (1:q))
  ...
  while(1-convergence){
    ...
    sample <- metropolis.HET(n, q, data=data, theta.k = theta.k, sigma.error=sigma.error, mu_f=mu_f, 
    ...
    uvector2 <- matrix(t(U),ncol=1,byrow=T) # Mnx1
  ...
  #########################################################
    ...
    {
      delta0<-varu[1] # GAR parameters for random effects cov. matrix 
      delta1<-varu[2]
      lambda0<-varu[3] # IV parameters for random effects cov. matrix
      lambda1<-varu[4]
      u.all <- matrix(NA,n,q)
      logdd <- matrix(NA,n,q)
      Sigma <- matrix(NA,n,q*q)
      for (i in 1:n){
        phi <- delta0*I+delta1*I*Zii[i,]
        T.hat <- phi+diag(1,q,q)
        T.hat[upper.tri(T.hat)]<-0
        T.hat[col(T.hat)<row(T.hat)] <- -1*(T.hat[col(T.hat)<row(T.hat)])
        sigma_sq <- sqrt(exp(lambda0+lambda1*Zii[i,]))
        D.hat <- diag(sigma_sq,q,q)
        Sigma <- t(solve(T.hat))%*%D.hat%*%solve(T.hat)
        mean <- rep(0,q)
        #library(mvtnorm)
        u.all[i,] <- rmvnorm(1,mean,Sigma)
        dd <- dmvnorm(u.all,mean,Sigma,log=TRUE)
      }
      sum(dd)/(n*q)
    }
  ...
  #########################################################
    ...
    random <- optim(fn=ll.random, par = c(theta.k[5],theta.k[6],theta.k[7],theta.k[8]),method = "L-BFGS-B",
                  lower=c(.01,.01,.01,.01),upper=c(3,3,3,3),control=list(fnscale=-1),hessian = TRUE)
    ifelse(random$par<0 | random$par>5,next,random$par)
    varu.new <-random$par
    # updated estimates for missing data parameter 
    alpha.update <- optim(fn = ll.alpha, par = c(theta.k[9],theta.k[10],theta.k[11],theta.k[12]),control=list(fnscale=-1),
    ...
    if (max(abs(c(theta.new[1]-theta.k[1],theta.new[2]-theta.k[2],theta.new[3]-theta.k[3],theta.new[4]-theta.k[4],
                  ...
                  theta.new[9]-theta.k[9],theta.new[10]-theta.k[10],theta.new[11]-theta.k[11],theta.new[12]-theta.k[12])))>tol){
      
      theta.k <-theta.new      
      beta <-c(theta.new[1],theta.new[2],theta.new[3],theta.new[4])
      varu <-c(theta.new[5],theta.new[6],theta.new[7],theta.new[8])
      alpha0<-theta.new[9]
      alpha <-c(theta.new[10],theta.new[11],theta.new[12])
      alphaA <- c(theta.new[9],theta.new[10],theta.new[11],theta.new[12])
    }
    else convergence <- T
  }
  if(convergence){
    ...
    se.beta <- sqrt(abs(diag(I.beta)))
    
    I.rand <- solve(-random$hessian)
    se.rand <- sqrt(abs(diag(I.rand)))
