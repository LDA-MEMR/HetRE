############################################################################
# Function for Metropolis-Hasting under HET model
############################################################################
#' @title Simulation of MCMC samples using Metropolis-Hasting under HET model
#'
...
metropolis.HET <- function (n, q, data, theta.k, sigma.error, mu_f, var_f, alpha0, alpha, M, burnin) 
{ 
  Ustar<-matrix(0,nrow= M,ncol=n*q) # Mxnq matrix to be filled with simulated u (random effects).
  xstar<-matrix(0,nrow= M,ncol=n*q) # Mxnq matrix to be filled with simulated x (measurement error covariate).
  ystar<-matrix(NA,nrow= M,ncol=n*q) # Mxnq matrix to be filled with simulated y
  ...
  ## Function for generating initial samples of random effects by proposed approach
  delta <- c(theta.k[5], theta.k[6])
  lambda <- c(theta.k[7], theta.k[8])
  Zii <- matrix(data$Zistar,n,q)
  Ustar[1,]<- u.sim.HET(n=n, q=q, delta = delta, Zi = Zii, lambda = lambda)$U.all # initial simulation of random effects.

  ## Generate initial error covariate x
  xstar[1,]<-rnorm(n*q, mu_f, sqrt(var_f)) # X ~ N(mu_f,sqrt(var_f)) ## initial simulation of X
  ...
  { 
    ...
    umat <-matrix(rep(Ustar[m-1,],1),n,q)# nxq; copies of previously simulated random effects.
    umat <-matrix(umat,n*q,1, byrow = T)
    ...
    eta.denom<- Fixed.denom + umat # nqx1, copies of linear predictors based on previously simulated u's.
    ...
    eta.numer <- Fixed.numer+ umat2 # nqx1; copies of linear predictors based on previously simulated u's.
